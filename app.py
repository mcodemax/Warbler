...
app.config['SQLALCHEMY_DATABASE_URI'] = (
    os.environ.get('DATABASE_URL', 'postgresql://postgres:myPassword@localhost:5433/warbler'))
...
def profile():
    ...
    #g.user.password = User.encode_pass(form.new_password.data) if form.new_password.data else g.user.password
    # possibly implement for pass change later
...
def homepage():
    ...
    - logged in: 100 most recent messages of followed_users
        only from the users that the logged-in user is following, and that user itself
    """
    # is_following()
    
    if g.user:
        follow_and_self_ids = [user.id for user in g.user.following] + [g.user.id]
               
        messages = (Message
                    .query
                    .filter(Message.user_id.in_(follow_and_self_ids))
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
